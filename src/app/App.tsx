        ...
        <main className="app-shell">
                ...
                onChange={(event) => {
                    ...
                    void analyzeImageWithGemini(file, (update) => {
                        ...
                        .catch((caught: unknown) => {
                            setAnalysisProgress(null);
                            setAnalysisStatus(caught instanceof Error ? caught.message : 'Image analysis failed.');
                        });
                    event.target.value = '';
                }}
            />
                ...
                <DetectionReviewModal
                    ...
                    onSave={async () => {
                        ...
                        try {
                            ...
                            setAnalysisStatus(
                                `${savedCount} item${savedCount === 1 ? '' : 's'} saved. Some items could not be saved.`,
                            );
                            await reload();
                        } finally {
                            setIsSaving(false);
            ...
            {confirmAction && (
                <ConfirmModal
                    ...
                    message={
                        confirmAction === 'clear'
                            ? 'This will permanently remove every saved item from this device.'
            ...
            {editingItem && (
                <ItemEditModal
                    ...
                    onSave={(changes) => {
                        setIsSaving(true);
                        void itemsRepository
                            .put({ ...editingItem, ...changes })
                            .then(async () => {
                                setEditingItem(null);
                                await reload();
                            })
