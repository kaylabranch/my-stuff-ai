    ...
    return (
        <section className="inventory-section">
            ...
            {isLoading ? (
                ...
                <div className="empty-state">
                    <Camera size={32} />
                    <h2>{items.length === 0 ? 'Your inventory is empty' : 'No items match your filters'}</h2>
                    <p>
                        {items.length === 0
                ...
                <div className={view === 'grid' ? 'inventory-grid' : 'inventory-list'}>
                    {visibleItems.map((item) => (
                        <article className="item-card" key={item.id}>
                            <div className="image-placeholder">
                                <InventoryImage blob={item.imageBlob} alt={item.name} />
