            ...
            <section className="review-modal" role="dialog" aria-modal="true" aria-labelledby="review-title">
                <div className="review-header">
                    <div>
                        ...
                        <h2 id="review-title">
                            Review {items.filter((item) => !item.removed).length} eligible objects
                        </h2>
                    </div>
                    <button
                ...
                <label className="review-room-field">
                    Room name (optional)
                    <input
                        value={roomName}
                        onChange={(event) => onRoomNameChange(event.target.value)}
                        placeholder="e.g. Living Room"
                        list="review-room-options"
                    />
                    <datalist id="review-room-options">
                        {rooms.map((room) => (
                ...
                {sourceUrl && (
                    <div className="review-source">
                        <img className="source-image" src={sourceUrl} alt="Uploaded room" />
                    </div>
                )}
                <div className="review-list">
                    {items.map((item, index) => (
                        <div
                            className={item.removed ? 'review-item removed' : 'review-item'}
                            key={`${item.name}-${index}`}
                        >
                            <div className="review-thumb">
                                {thumbUrls[index] ? (
                                    <img src={thumbUrls[index] || undefined} alt={`Crop of ${item.name}`} />
                                ) : (
                                    <Camera size={20} />
                                )}
                                <span title="AI Confidence Score">{Math.round(item.confidence * 100)}%</span>
                            </div>
                            <div className="review-fields">
                                <input
                                    aria-label={`Object ${index + 1} name`}
                                    value={item.name}
                                    onChange={(event) => onChange(index, { name: event.target.value })}
                                />
                                <div className="review-controls">
                                    <select
                                        aria-label={`Object ${index + 1} category`}
                                        value={item.category}
                                        onChange={(event) =>
                                            onChange(index, {
                                                category: event.target.value as DetectedItem['category'],
                                            })
                                        }
                                    >
                                    ...
                                    <input
                                        aria-label={`Object ${index + 1} tags`}
                                        value={item.suggestedTags.join(', ')}
                                ...
                                <textarea
                                    aria-label={`Object ${index + 1} description`}
                                    value={item.description}
                                    rows={2}
                                    onChange={(event) => onChange(index, { description: event.target.value })}
                                />
                                <label className="review-value">
                                    <span>Est. value</span>
                                    <span className="currency-input">
                                        <span>$</span>
                                        <input
                                            aria-label={`Object ${index + 1} estimated value`}
                                            type="number"
                                            min="0"
                                            step="0.01"
                            ...
                            <button
                                ...
                                aria-label={item.removed ? `Restore ${item.name}` : `Remove ${item.name}`}
                                title={item.removed ? 'Restore item' : 'Remove item'}
                            >
                                {item.removed ? <Upload size={17} /> : <Trash2 size={17} />}
                            </button>
                        </div>
                    ))}
                </div>
                <div className="review-footer">
                    <button className="secondary-button" type="button" onClick={onClose} title="Discard this batch">
                        Discard
                    </button>
                    <button
                        ...
                        onClick={onSave}
                        title="Save eligible items"
                    >
                        <Save size={17} /> {isSaving ? 'Saving...' : `Save ${eligibleCount} items`}
                    </button>
                </div>
            </section>
        </div>
