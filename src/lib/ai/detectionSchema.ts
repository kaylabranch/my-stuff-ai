...
function normalizeBoundingBox(value: unknown) {
    ...
    if ('left' in box && 'top' in box && 'width' in box && 'height' in box) {
        return {
            x: asNumber(box.left, 0),
            y: asNumber(box.top, 0),
            w: asNumber(box.width, 1),
            h: asNumber(box.height, 1),
        };
    }
    if (['x1', 'y1', 'x2', 'y2'].every((key) => key in box)) {
        return {
            x: asNumber(box.x1, 0),
            y: asNumber(box.y1, 0),
            w: asNumber(box.x2, 1) - asNumber(box.x1, 0),
            h: asNumber(box.y2, 1) - asNumber(box.y1, 0),
        };
    }
    if (
        ['xmin', 'ymin', 'xmax', 'ymax'].every((key) => key in box) ||
        ['left', 'top', 'right', 'bottom'].every((key) => key in box)
    ) {
...
function normalizeItem(value: unknown) {
    ...
    const scale =
        Math.max(Math.abs(box.x), Math.abs(box.y), Math.abs(box.w), Math.abs(box.h)) > 1
            ? Math.max(Math.abs(box.x), Math.abs(box.y), Math.abs(box.w), Math.abs(box.h)) <= 100
                ? 100
                : 1000
            : 1;
    return {
