...
export async function cropImageToBlob(file: File, boundingBox: BoundingBox): Promise<Blob> {
    ...
    try {
        ...
        context.drawImage(image, x * image.naturalWidth, y * image.naturalHeight, Math.max(0.01, right - x) * image.naturalWidth, Math.max(0.01, bottom - y) * image.naturalHeight, 0, 0, canvas.width, canvas.height);
